...
class FavoritesVC: UIViewController
{
    ...
    func viewfavorites(offset:Int)
    {
        ...
        Api.Viewfavouriteinmylibrary(UserDefaults.standard.string(forKey: "id") ?? "", limit: 20, offset: index, endpoint: ApiEndPoints.ViewFavourite, vc: self) { (res, err) -> (Void) in
                  do
                  {
                    ...
                    if let total = Int(self.FavsMoviedata?.total_content ?? "")
                    {
                        self.totalconents = total
                        print("total ---- \(self.totalconents) ")
                    }
                    for item in self.FavsMoviedata?.subComedymovList ?? [newFilteredSubComedymovieList]()
                    {
...
extension FavoritesVC:UICollectionViewDelegate,UICollectionViewDataSource
{
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        {
            self.ComedyCollectionV.setEmptyViewnew1(title: "No Favorites found")
        }else
        {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath)
    {
        ...
        getcontentauthorized(movieid: FavsMoviedata?.subComedymovList?[indexPath.row].movie_uniq_id ?? "", planurls:"", index: indexPath.row, permad: FavsMoviedata?.subComedymovList?[indexPath.row].c_permalink ?? "", isbanner: 3)
    }
