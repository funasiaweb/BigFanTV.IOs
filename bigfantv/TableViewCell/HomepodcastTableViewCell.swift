...
import UIKit
import SDWebImage
protocol CollectionViewCellDelegate11: class {
...
class HomepodcastTableViewCell: UITableViewCell {
    
    weak var cellDelegate: CollectionViewCellDelegate11?
    
    var rowWithColors: [PodcastRssfeeddatadetails]?
    var rowindext = 0
...
extension HomepodcastTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    
    // The data we passed from the TableView send them to the CollectionView Model
    func updateCellWith(row: [PodcastRssfeeddatadetails],rowindex:Int) {
        rowindext = rowindex
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "collectionviewcellid", for: indexPath) as? CollectionViewCell {
            ...
            cell.Imageview.sd_setImage(with: URL(string: "\(self.rowWithColors?[indexPath.item].image ?? "")"), completed: nil)
