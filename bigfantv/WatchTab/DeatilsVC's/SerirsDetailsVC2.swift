...
import MuviPlayer
import CoreMedia
class SerirsDetailsVC2: UIViewController,UIPopoverPresentationControllerDelegate {
   ...
   extension SerirsDetailsVC2:MDCTabBarDelegate
   {
   ...
   extension SerirsDetailsVC2:UICollectionViewDelegate,UICollectionViewDataSource
   {
        ...
        if Episodesdata.count < totalconents
           {
            ...
            if indexPath.row == Episodesdata.count - 1 && !(isloading ?? false)
               {  //numberofitem count
                   isloading = true
                  index = index + 1
                  self.getEpisodedeatils(permaa: perma, offset: index)
   ...
   extension SerirsDetailsVC2: UICollectionViewDelegateFlowLayout {
...
extension SerirsDetailsVC2:MuviPlayerPlaybackDelegate
{
